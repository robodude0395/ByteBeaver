...
describe('AgentClient', () => {
    ...
    beforeEach(() => {
        ...
        client = new agentClient_1.AgentClient('http://localhost:8000');
    });
    afterEach(() => {
        jest.resetAllMocks();
    });
    ...
    describe('getStatus', () => {
        ...
        it('throws meaningful error on 404', async () => {
            const axiosError = new axios_1.default.AxiosError('Not Found');
            axiosError.response = { status: 404, data: { detail: 'Session not found' } };
            mockAxiosInstance.get.mockRejectedValue(axiosError);
            await expect(client.getStatus('bad-id')).rejects.toThrow('Failed to get session status: Server returned 404 - Session not found');
        });
    });
    ...
    describe('error handling', () => {
        it('gives clear error on connection refused', async () => {
            ...
            mockAxiosInstance.post.mockRejectedValue(axiosError);
            await expect(client.sendPrompt('test', '/ws')).rejects.toThrow('Failed to send prompt: Agent server is not reachable. Is it running?');
        });
        it('gives clear error on timeout', async () => {
            const axiosError = new axios_1.default.AxiosError('timeout');
            axiosError.code = 'ECONNABORTED';
            mockAxiosInstance.post.mockRejectedValue(axiosError);
            await expect(client.sendPrompt('test', '/ws')).rejects.toThrow('Failed to send prompt: Request timed out. The server may be overloaded.');
        });
        it('includes detail from 500 server error', async () => {
            const axiosError = new axios_1.default.AxiosError('Internal Server Error');
            axiosError.response = { status: 500, data: { detail: 'LLM server crashed' } };
            mockAxiosInstance.post.mockRejectedValue(axiosError);
            await expect(client.sendPrompt('test', '/ws')).rejects.toThrow('Failed to send prompt: Server returned 500 - LLM server crashed');
        });
    });
});
//# sourceMappingURL=agentClient.test.js.map